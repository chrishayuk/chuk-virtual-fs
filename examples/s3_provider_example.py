...
async def cleanup_demo_data(vfs):
    ...
    try:
        ...
        for file_path in known_files:
            try:
                if await vfs.exists(file_path) and await vfs.rm(file_path):
                    deleted_count += 1
            except Exception:
                pass
...
"""
    ...
    try:
        url = await vfs.generate_presigned_url(
            "/data/exports/sensor_data.csv", expires_in=3600
        )
        if url:
            print("  ✓ Generated presigned URL (expires in 1 hour):")
    ...
    # 12. Streaming with progress reporting and atomic writes
    print("\n12. Streaming large files with progress tracking:")
    ...
    def track_progress(bytes_written, total_bytes):
        ...
        if bytes_written % (100 * 1024) < 1024:
            print(f"  Progress: {bytes_written / 1024:.1f} KB uploaded...")

    # Generate large file for S3
    ...
    print(f"  ✓ Streaming complete: {progress_data['bytes'] / 1024:.1f} KB uploaded")
    ...
    if node_info:
        print(f"  ✓ Final file size: {node_info.size / 1024:.1f} KB in S3")

    # 13. Final S3 structure
    print("\n13. Final S3 structure:")
    await list_s3_tree("/")

    # 14. Final cleanup - remove all demo data
    print("\n14. Final cleanup - removing all demo data...")
