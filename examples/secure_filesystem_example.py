...
async def demonstrate_secure_operations(secure_fs, config_name):
    ...
    try:
        ...
        if result and not secure_fs.read_only:
            write_result = await secure_fs.write_file(f"{allowed_path}/test.txt", b"Hello secure world!")
    ...
    try:
        ...
        print(f"   Unauthorized path blocked: ✅")
    ...
    print(f"   Size: {stats.get('total_size', 0)}/{stats.get('max_total_size', 'N/A')} bytes")
...
async def run_security_demonstration():
    ...
    for config_name, config in configs.items():
        with tempfile.TemporaryDirectory(prefix=f"secure_fs_{config_name}_") as temp_dir:
            ...
            try:
                ...
                violations_after = await demonstrate_secure_operations(secure_fs, config_name)
...
async def demonstrate_integration_with_fs_manager():
    ...
    with tempfile.TemporaryDirectory(prefix="secure_vfs_") as temp_dir:
        ...
        try:
            ...
            await secure_provider.create_node(EnhancedNodeInfo("test.txt", False, "/safe"))
            await secure_provider.write_file("/safe/test.txt", b"Secure content")
            
            content = await secure_provider.read_file("/safe/test.txt")
            print(f"✅ Secure VFS integration: {content.decode() if content else 'None'}")
