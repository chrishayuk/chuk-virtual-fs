"""
debug/snapshots_example_usage.py - Example usage of the enhanced virtual filesystem with snapshots and templates
"""
...
def example_snapshot_usage(fs):
    ...
    initial_snapshot = snapshot_mgr.create_snapshot(
        "initial_state", "Initial filesystem setup"
    )
    print(f"Created snapshot: {initial_snapshot}")

    # Make some changes
    fs.mkdir("/home/user/projects/python")
    fs.write_file("/home/user/projects/python/hello.py", 'print("Hello")')
    fs.rm("/home/user/hello.txt")

    # List current files
    print("\nCurrent files after changes:")
    current_paths = list_files_recursively(fs, "/home/user")
    for path in current_paths:
        print(path)

    # Create another snapshot
    modified_snapshot = snapshot_mgr.create_snapshot(
        "modified_state", "After adding project files"
    )
    ...
    initial_paths = list_files_recursively(fs, "/home/user")
    ...
    modified_paths = list_files_recursively(fs, "/home/user")
    for path in modified_paths:
        print(path)


# Example 2: Template loading
def example_template_usage(fs):
    ...
    project_paths = list_files_recursively(fs, "/home/user/project")
    for path in project_paths:
        print(path)

    # Display README content
    print("\nREADME.md content:")
    readme_content = fs.read_file("/home/user/project/README.md")
    ...
    final_project_paths = list_files_recursively(fs, "/home/user/project")
    for path in final_project_paths:
        print(path)


# Example 3: Combined usage for a sandbox environment
def example_sandbox_environment(fs):
    ...
    initial_snapshot = snapshot_mgr.create_snapshot(
        "sandbox_initial", "Initial sandbox setup"
    )
    print(f"Created initial sandbox snapshot: {initial_snapshot}")

    # Add a shared resource
    fs.write_file("/shared/utils.py", "Common utility functions for assignments")

    # Simulate student working on assignment
    fs.write_file(
        "/home/student/assignments/assignment1.py",
        "def solve_problem():\n    return 42",
    )

    # Create a snapshot after student work
    student_work_snapshot = snapshot_mgr.create_snapshot(
        "student_work", "Student progress on assignment"
    )
    print(f"Created student work snapshot: {student_work_snapshot}")

    # List available files
    print("\nSandbox environment structure:")
    sandbox_paths = list_files_recursively(fs, "/home/student")
    shared_paths = list_files_recursively(fs, "/shared")
    ...
    restored_paths = list_files_recursively(fs, "/home/student")
    restored_shared_paths = list_files_recursively(fs, "/shared")
    print("Student directory:")
...
# Run the examples
def main():
    # Create a virtual filesystem directly
    fs = VirtualFileSystem()

    # Run the examples with the filesystem
    example_snapshot_usage(fs)
    example_template_usage(fs)
    example_sandbox_environment(fs)
