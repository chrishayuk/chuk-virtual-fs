...
def default_security_example():
    ...
    # Attempt to demonstrate security violations
    print("\n1. Attempting to write to a protected system path...")
    print("   Trying to modify /etc/passwd:")
    result = fs.write_file("/etc/passwd", "root:x:0:0:")
    print(f"   Write result: {'Success' if result else 'Failed (as expected)'}")
    
    print("\n2. Attempting to create a file with a dangerous pattern:")
    print("   Trying to create file with path traversal pattern:")
    result = fs.touch("/home/user/../etc/sensitive")
    print(f"   File creation result: {'Success' if result else 'Failed (as expected)'}")
    
    # Retrieve and display security violations
...
def advanced_quota_management_example():
    ...
    # Create filesystem with the untrusted profile (tight restrictions)
    fs = VirtualFileSystem(security_profile="untrusted")
    
    # Retrieve and display current quota settings
...
def custom_security_configuration_example():
    ...
    # Create filesystem with custom security parameters
    ...
    # Demonstrate path and file type restrictions
    print("\nDemonstrating Restrictions:")
    
    print("1. Writing to allowed path:")
    result = fs.write_file("/projects/notes.txt", "Project documentation")
    print(f"   Result: {'Success' if result else 'Failed'}")
    
    print("\n2. Attempting to write outside allowed paths:")
    result = fs.write_file("/home/unauthorized.txt", "Unauthorized file")
    print(f"   Result: {'Success' if result else 'Failed (as expected)'}")
    
    print("\n3. Attempting to create denied file types:")
...
def security_violation_management_example():
    ...
    # Demonstrate clearing violations
