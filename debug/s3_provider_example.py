            ...
            if "404" in str(e):
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            f"Bucket created with region constraint: {create_response}"
                        )
                        return True
                    except Exception as region_e:
                        print(
                            f"Failed to create bucket with region constraint: {region_e}"
                        )
                        return False
            else:
...
def cleanup_prefix(bucket_name, prefix, endpoint_url=None):
    ...
    try:
        ...
        print(
            f"Cleanup complete: {total_deleted}/{total_objects} objects deleted from prefix '{prefix}'"
        )
        return True
    except Exception as e:
...
def basic_s3_example():
    ...
    try:
        ...
        fs.write_file(
            "/data/sample.txt", "This is sample data stored in the S3 bucket."
        )
...
def test_root_directory_files():
    ...
    try:
        # Create filesystem with S3 provider
        fs = VirtualFileSystem(
            "s3", bucket_name=bucket_name, prefix=prefix, endpoint_url=endpoint_url
        )
...
2023-01-05,12""",
        ...
        print(
            f"Original file exists: {fs.get_node_info('/web_project/data.csv') is not None}"
        )
        print(
            f"Copy exists in new location: {fs.get_node_info('/data/backup.csv') is not None}"
        )
...
def s3_snapshots_demo():
    ...
    try:
        ...
        initial_snapshot = snapshot_mgr.create_snapshot(
            "initial_state", "Initial file setup"
        )
        ...
        fs.write_file(
            ...
            '{"debug": true, "version": "1.0.1", "mode": "testing"}',
        )
        ...
        modified_snapshot = snapshot_mgr.create_snapshot(
            "modified_state", "After file modifications"
        )
        ...
        for snap in snapshots:
            print(
                f"  {snap['name']}: {snap['description']} (created: {snap['created']})"
            )
...
def main():
    ...
    if not bucket_created:
        print(
            "Warning: Could not create bucket. Examples may fail if bucket doesn't exist."
        )
