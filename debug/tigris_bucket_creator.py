        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"Attempting to create bucket with LocationConstraint={region}")
                    s3.create_bucket(
                        Bucket=bucket_name,
                        CreateBucketConfiguration={'LocationConstraint': region}
                    )
                    logger.info(f"Successfully created bucket '{bucket_name}' with region constraint")
                    return True
                    ...
                    try:
                        ...
                        logger.info("Attempting to create bucket with empty configuration")
                        s3.create_bucket(
                            Bucket=bucket_name,
                            CreateBucketConfiguration={}
                        )
                        logger.info(f"Successfully created bucket '{bucket_name}' with empty config")
                        return True
                    except Exception as e3:
                        logger.error(f"All bucket creation methods failed. Last error: {e3}")
                        return False
