...
class TestS3StorageProviderNew:
    ...
    async def test_directory_listing_after_file_creation(self, initialized_provider):
        ...
        files = await provider.list_directory("/data/logs")
    ...
    async def test_initialization_import_error(self):
        ...
        with patch('builtins.__import__', side_effect=ImportError("No module named 'aioboto3'")):
    ...
    async def test_client_with_custom_endpoint(self):
        ...
        with patch("aioboto3.Session") as mock_session:
            ...
            async with provider._get_client() as client:
                pass
    ...
    async def test_create_directory(self, initialized_provider):
        ...
        result = await provider.create_directory("/test/dir2", mode=0o755, owner_id=1001, group_id=1001)
        assert result is True
    ...
    async def test_delete_directory_recursive(self, initialized_provider):
        ...
        with patch.object(provider, "_is_directory", return_value=True):
            ...
            with patch.object(provider, "list_directory", return_value=["file1.txt", "file2.txt"]):
    ...
    async def test_generate_presigned_upload_url_full(self, initialized_provider):
        ...
        result = await provider.generate_presigned_upload_url("/upload.txt", expires_in=1800)
        ...
        mock_client.generate_presigned_post.assert_called_once_with(
            Bucket="test-bucket",
            Key="test-prefix/upload.txt",
            ExpiresIn=1800
        )
    
    @pytest.mark.asyncio
    async def test_generate_presigned_upload_url_with_content_type(self, initialized_provider):
    ...
    async def test_generate_presigned_upload_url_error(self, initialized_provider):
        ...
        mock_client.generate_presigned_post = AsyncMock(side_effect=Exception("S3 Error"))
    ...
    async def test_generate_presigned_url_error(self, initialized_provider):
        ...
        mock_client.generate_presigned_url = AsyncMock(side_effect=Exception("S3 Error"))
    ...
    async def test_get_node_info_exception_handling(self, initialized_provider):
        ...
        with patch.object(provider, "_is_directory", side_effect=Exception("Directory check error")):
        ...
        async def mock_paginate(**kwargs):
            yield {
                "Contents": [
                    ...
                    {"Key": "test-prefix/dir/nested/deep.txt", "Size": 200},  # Should be skipped
    ...
    async def test_copy_node_directory(self, initialized_provider):
        ...
        with patch.object(provider, "_is_directory", side_effect=mock_is_directory):
            with patch.object(provider, "list_directory", side_effect=mock_list_directory):
                with patch.object(provider, "create_directory", side_effect=mock_create_directory):
                    ...
                    with patch.object(provider, "copy_node", side_effect=mock_copy_node):
                        result = await provider.copy_node("/source/dir", "/dest/dir")
    ...
    async def test_get_object_tags(self, initialized_provider):
        ...
        mock_client.get_object_tagging = AsyncMock(return_value={
            "TagSet": [
    ...
    async def test_create_directory_with_marker(self, initialized_provider):
        ...
        result = await provider.create_directory("/test/dir", mode=0o755, owner_id=1000, group_id=1000)
        ...
        async def mock_paginate(**kwargs):
            yield {
                "Contents": [
                    ...
                    {"Key": "test-prefix/dir1/file2.txt", "Size": 200},  # File in directory
                    {"Key": "test-prefix/dir1/subdir/", "Size": 0},  # Nested directory marker
                ]
            }
    ...
    async def test_create_node_directory(self, initialized_provider):
        ...
        with patch.object(provider, "create_directory", return_value=True) as mock_create:
            result = await provider.create_node(node_info)
